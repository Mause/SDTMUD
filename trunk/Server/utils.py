...
def proccess(data, clientsocket, instance):
	...
	if data != '':
		...
		if (data.split()[0]) == 'help':
			print '['+str(time.ctime())+'] Quick! Someone needs help!'
...
* "exit": kills your personalised instance of the server.\n''')
			...
			print '['+str(time.ctime())+'] Giving the menu to the user...'
			msg = ''
			...
			if len(data.split()[1:]) == '' or len(data.split()) == 1:
				print '['+str(time.ctime())+'] The user forgot the mud...'
				clientsocket.send('Please specify a mud to load.\n')
			else:
				print '['+str(time.ctime())+'] Serving the dish to the user...'
				for x in range(len(config.get('mudlist', 'list'))):
...
def run_loop(clientsocket, instance, path_to_mud, run_script):
	...
	print '['+str(time.ctime())+'] The beginning result was', result
	while result != '':
		if result == 'exit':
			print '['+str(time.ctime())+'] exit token/command recieved from client, closing connection and telling client to hit enter to exit'
			return 'exit'
		else:
			print '['+str(time.ctime())+'] About to do these things:'
			print '['+str(time.ctime())+'] import '+str(path_to_mud[1:]+'.'+result+' as '+(result.split('.')[-1:][0]))
			print '['+str(time.ctime())+'] result = next_room.run_room(clientsocket, instance)'
